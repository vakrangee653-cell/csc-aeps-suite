...
import { 
  ...
  Briefcase, Edit3, Trash2, Milestone, ArrowRightLeft, DollarSign, ShieldAlert
} from 'lucide-react';
import { Transaction, Customer, ServiceItem, WalletTransaction, User } from '../types';
import { getGlobalInactiveServiceIds } from '../utils';
...
export default function CspManager({
  activeUser, transactions, customers, services, walletTransactions, walletBalance,
  ...
  const [custServiceId, setCustServiceId] = useState(() => {
    const inactive = getGlobalInactiveServiceIds();
    const firstActive = services.find(s => !inactive.includes(s.id));
  ...
  const [customerWorkFilter, setCustomerWorkFilter] = useState<'all' | 'Pending' | 'Processing' | 'Complete'>('all');
  const [customerDuesFilter, setCustomerDuesFilter] = useState<'all' | 'dues' | 'no-dues'>('all');
  ...
  const filteredCustomersList = customers.filter(c => {
    ...
    const category = srv ? srv.category : 'Other';

    const matchesSearch = c.name.toLowerCase().includes(searchCustomers.toLowerCase()) ||
                          c.phone.includes(searchCustomers) ||
                          c.serviceName.toLowerCase().includes(searchCustomers.toLowerCase());
                          
    const matchesWork = customerWorkFilter === 'all' || c.workStatus === customerWorkFilter;
    const matchesDues = customerDuesFilter === 'all' || 
                        (customerDuesFilter === 'dues' && c.dues > 0) || 
                        (customerDuesFilter === 'no-dues' && c.dues === 0);
                        
    const matchesCategory = customerCategoryFilter === 'all' || category === customerCategoryFilter;

    return matchesSearch && matchesWork && matchesDues && matchesCategory;
  });
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="form-group">
                  ...
                  <select value={custServiceId} onChange={(e) => setCustServiceId(e.target.value)}>
                    {services.filter(s => !getGlobalInactiveServiceIds().includes(s.id)).map(s => (
              ...
              <div className="flex justify-between items-center border-b border-gray-100 dark:border-gray-800 pb-2 flex-wrap gap-2">
                ...
                <div className="relative">
                  <input 
                    ...
                    className="p-1.5 pl-8 border rounded-lg text-xs bg-gray-50/50 dark:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500"
                  />
                  <Search className="w-3.5 h-3.5 text-gray-400 absolute left-2.5 top-1/2 -translate-y-1/2" />
                </div>
              </div>

              {/* ADVANCED MULTI-CRITERIA FILTERS FOR CUSTOMERS */}
              <div className="bg-gray-50/50 dark:bg-slate-900/50 border border-gray-100/70 dark:border-gray-800/60 p-4 rounded-xl mt-3 flex flex-col gap-3 text-left animate-fade-in select-none">
                
                {/* Categorization filter Row */}
                <div className="flex flex-col gap-1.5">
                  <span className="text-[10px] font-black uppercase text-gray-400 dark:text-gray-500 tracking-wider flex items-center gap-1">
                    📂 सेवा की श्रेणी (Service Category)
                  </span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-1">
                  ...
                  <div className="flex flex-col gap-1.5">
                    <span className="text-[10px] font-black uppercase text-gray-400 dark:text-gray-500 tracking-wider flex items-center gap-1">
                      ⚙️ कार्य की प्रगति (Work Status)
                    </span>
                    <div className="flex flex-wrap gap-1">
                      {[
                        { value: 'all', label: 'सभी कार्य' },
                        { value: 'Pending', label: 'लंबित' },
                        { value: 'Processing', label: 'प्रगति पर' },
                        { value: 'Complete', label: 'पूर्ण' }
                      ].map(opt => (
                        <button
                          type="button"
                          key={opt.value}
                          onClick={() => setCustomerWorkFilter(opt.value as any)}
                          className={`px-3 py-1 rounded-full text-[10px] font-black transition-all cursor-pointer ${
                            customerWorkFilter === opt.value
                  ...
                  <div className="flex flex-col gap-1.5">
                    <span className="text-[10px] font-black uppercase text-gray-400 dark:text-gray-500 tracking-wider flex items-center gap-1">
                      💰 भुगतान बकाया (Dues Filter)
                    </span>
                    <div className="flex flex-wrap gap-1">
                      {[
                        { value: 'all', label: 'सभी ग्राहक' },
                        { value: 'dues', label: 'केवल बकाया' },
                        { value: 'no-dues', label: 'शून्य बकाया' }
                      ].map(opt => (
                        <button
                          type="button"
                          key={opt.value}
                          onClick={() => setCustomerDuesFilter(opt.value as any)}
                          className={`px-3 py-1 rounded-full text-[10px] font-black transition-all cursor-pointer ${
                            customerDuesFilter === opt.value
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mt-2">
            {filteredServices.map(srv => {
              ...
              const isInactive = getGlobalInactiveServiceIds().includes(srv.id);
              return (
                <div key={srv.id} className={`p-5 rounded-2xl bg-white dark:bg-slate-900/40 border relative flex flex-col justify-between ${isInactive ? 'border-red-300 dark:border-red-900/40 bg-red-50/10 opacity-85' : 'border-gray-100 dark:border-gray-800'}`}>
                  <div>
          ...
          <div className="tool-layout">
            <div className="config-card">
              ...
              {activeUser?.role === 'Admin' && (
                <div className="mt-5 pt-4 border-t border-red-500/25 bg-red-100/5 dark:bg-red-950/10 p-4 rounded-xl flex flex-col gap-2 border border-red-200">
                  <div className="flex items-center gap-2 text-red-600 dark:text-red-400 font-extrabold text-xs">
                    <ShieldAlert className="w-4 h-4 shrink-0" />
                    <span>Super Admin Controls (सर्वोच्च व्यवस्थापक अधिकार)</span>
                  </div>
                  <p className="text-[10px] text-gray-500 dark:text-gray-400 leading-snug">
                    आप एडमिन हैं! आप इस वॉलेट बैलेंस तथा सभी लोड या डेबिट इतिहास को साफ़ करके इसे तत्काल <strong>₹0.00</strong> पर रीसेट कर सकते हैं।
                  </p>
                  <button 
                    type="button"
                    onClick={() => {
                      if (confirm('क्या आप सचमुच वॉलेट को रीसेट करना चाहते हैं? इससे सभी वॉलेट लोड और उपयोग का इतिहास मिट जायेगा और बैलेंस ₹0.00 हो जायेगा।')) {
                        onResetWallet?.();
                      }
                    }}
                    className="w-full bg-red-600 hover:bg-red-700 text-white rounded-xl py-2 px-3 text-[10px] font-black tracking-wide cursor-pointer transition-all uppercase flex items-center justify-center gap-1.5 shadow-sm shadow-red-500/10"
                  >
                    <Trash2 className="w-3.5 h-3.5" />
                    <span>Reset Wallet Balance to ₹0</span>
                  </button>
                </div>
              )}
