...
export default function StaffManager({ activeUser, onStaffUpdated }: StaffManagerProps) {
  ...
  const handleCreateStaff = (e: React.FormEvent) => {
    ...
    // Owner is only allowed to create Staff users
    const finalRoleToCreate = activeUser.role === 'Owner' ? 'Staff' : role;
    
    // Check if the pin is already registered for this role
    const existing = currentUsers.find(u => u.pin === pin && u.role === finalRoleToCreate);
    if (existing) {
  ...
  // Filter staffList based on role
  // Super Admin (Admin) sees and controls everyone (Admins, Owners, Staff)
  // Owner only sees and controls Staff users
  const rawUsers = getStaffUsers();
  const staffList = activeUser.role === 'Admin' 
    ? rawUsers 
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="panel-header">
        <h2 className="panel-title">
          ...
          {activeUser.role === 'Admin' ? 'Global Admin User Directory' : 'Staff Directory & Access Control'}
        </h2>
      </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="form-group">
                ...
                {activeUser.role === 'Owner' ? (
                  <div className="p-2 bg-gray-100 dark:bg-slate-900/60 border border-gray-200 dark:border-slate-800 rounded-xl text-xs font-bold text-blue-500 dark:text-blue-400">
                    Staff (Operator permissions only)
                  </div>
                ) : (
