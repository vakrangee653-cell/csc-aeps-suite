...
export default function Sidebar({ 
  ...
  const isRestrictedForStaff = (tab: string) => {
    const restricted = ['profile', 'csp', 'reports', 'services', 'staff', 'admin', 'developer'];
  ...
  return (
    <aside className={`sidebar ${isCollapsed ? 'collapsed' : ''}`} id="sidebar">
      ...
      <nav className="nav-links my-4">
        ...
        >
          <Crown className="w-5 h-5 shrink-0 text-amber-500 animate-pulse" />
        ...
        {userRole === 'Admin' && (
          <a 
            className={`nav-item flex items-center gap-2 p-2.5 rounded-xl border border-amber-500/25 bg-amber-500/5 hover:bg-amber-500/10 text-amber-500 font-bold transition-all my-1.5 ${currentTab === 'developer' ? 'active bg-gradient-to-r from-amber-500 to-yellow-500 text-slate-950 font-black border-none' : ''}`}
            onClick={() => handleTabClick('developer')}
            title="Platform-wide Developer Administration Panel"
          >
            <Crown className={`w-5 h-5 shrink-0 animate-pulse ${currentTab === 'developer' ? 'text-slate-950' : 'text-amber-500'}`} />
      ...
      <div className="sidebar-footer">
        <a 
          ...
          className="theme-toggle-btn bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 border border-emerald-500/15 dark:border-emerald-500/25 text-xs font-bold py-2 justify-center flex items-center gap-2 rounded-xl transition-all hover:scale-[1.02]"
          title="Directly download clean project source code ZIP"
        >
