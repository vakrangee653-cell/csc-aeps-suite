...
export default function Dashboard({
  ...
  const [searchWorkFilter, setSearchWorkFilter] = useState<'all' | 'Pending' | 'Processing' | 'Complete'>('all');
  const [searchDuesFilter, setSearchDuesFilter] = useState<'all' | 'dues' | 'no-dues'>('all');
  ...
  useEffect(() => {
    if (services.length > 0) {
      ...
      if (firstActive) {
        setCustServiceId(firstActive.id);
      } else {
        setCustServiceId('manual');
      }
    }
  }, [services]);
  ...
  const matchedCustomers = customers.filter(c => {
    ...
    const matchesWork = searchWorkFilter === 'all' || c.workStatus === searchWorkFilter;
    const matchesDues = searchDuesFilter === 'all' || 
                        (searchDuesFilter === 'dues' && c.dues > 0) || 
                        (searchDuesFilter === 'no-dues' && c.dues === 0);

    return matchesSearch && matchesWork && matchesDues;
  });
              ...
              <div className="form-group">
                ...
                <div className="relative">
                  <input 
                    ...
                    className="p-2 pl-10 border rounded-md w-full animate-fade-in"
                  />
                  <Search className="w-4 h-4 text-gray-400 absolute left-3.5 top-1/2 -translate-y-1/2" />
                </div>
              </div>

              {/* DYNAMIC MULTI-SEGMENT CUSTOMER SELECTION FILTERS */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pb-2 select-none text-left bg-gray-50/40 dark:bg-slate-900/40 p-3 rounded-xl border border-gray-100 dark:border-gray-800/60">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {[
                      ...
                      <button
                        type="button"
                        key={opt.value}
                        onClick={() => setSearchWorkFilter(opt.value as any)}
                        className={`px-2.5 py-1 rounded-full text-[10px] font-black transition-all cursor-pointer ${
                          searchWorkFilter === opt.value
                            ? 'bg-blue-600 text-white shadow-sm'
                            : 'bg-gray-150 dark:bg-slate-800 text-gray-500 hover:text-gray-900 dark:hover:text-white'
                        }`}
                      >
                        {opt.label}
                      </button>
                ...
                <div className="flex flex-col gap-1.5">
                  <span className="text-[10px] font-extrabold text-gray-400 uppercase tracking-wider flex items-center gap-1">
                    ...
                    भुगतान बकाया (Payment Status)
                  </span>
                  <div className="flex flex-wrap gap-1">
                    {[
                      { value: 'all', label: 'सभी ग्राहक' },
                      { value: 'dues', label: 'केवल बकाया' },
                      { value: 'no-dues', label: 'पूर्ण भुगतान' }
                    ].map(opt => (
                      <button
                        type="button"
                        key={opt.value}
                        onClick={() => setSearchDuesFilter(opt.value as any)}
                        className={`px-2.5 py-1 rounded-full text-[10px] font-black transition-all cursor-pointer ${
                          searchDuesFilter === opt.value
              ...
              <div className="max-h-56 overflow-y-auto flex flex-col gap-2 pr-1 select-none">
                {matchedCustomers.length === 0 ? (
                  <div className="text-center py-6 text-xs text-gray-400">
                    {searchQuery.trim() === '' ? 'चयनित फ़िल्टर के साथ कोई भी ग्राहक पंजीकृत नहीं है' : 'खोज से मेल खाते हुए कोई ग्राहक नहीं मिले (No matches found)'}
                  </div>
                ) : (
