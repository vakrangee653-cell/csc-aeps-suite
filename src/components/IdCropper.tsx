...
interface IdCropperProps {
  activePlan?: 'Free' | 'Basic' | 'Premium';
  onNavigateToSubscriptions?: () => void;
}

export default function IdCropper({ activePlan = 'Free', onNavigateToSubscriptions }: IdCropperProps) {
  const [isServiceFree, setIsServiceFree] = useState(false);

  useEffect(() => {
    const checkServiceMode = () => {
      const modes = getStoredData<{ [key: string]: 'Free' | 'Paid' }>('csc_service_modes', {
        aadhaar: 'Paid',
        biodata: 'Paid',
        reports: 'Paid'
      });
      setIsServiceFree(modes.aadhaar === 'Free');
    };

    checkServiceMode();
    window.addEventListener('storage', checkServiceMode);
    return () => window.removeEventListener('storage', checkServiceMode);
  }, []);
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {(!isServiceFree && activePlan !== 'Premium') ? (
        <div className="config-card p-8 text-center flex flex-col items-center justify-center gap-5 border border-dashed border-amber-500/30 bg-gradient-to-b from-slate-900/10 to-slate-950/20 rounded-2xl min-h-[400px]">
          ...
          <div className="max-w-md">
            <h3 className="text-xl font-extrabold text-gray-900 dark:text-white">🔒 Premium VIP Pro Upgrade Required</h3>
            <p className="text-xs text-slate-400 leading-relaxed mt-2">
              Aadhaar & Voter ID Card High-Definition Alignment (क्रॉपिंग और ऑटो साइज़िंग लेआउट) केवल हमारे **Premium VIP Pro** ग्राहकों के लिए उपलब्ध है।
            </p>
            <p className="text-xs text-slate-400 leading-relaxed mt-1">
              अपने केंद्र को आज ही अपग्रेड करें और तुरंत सभी फीचर्स चालू करें!
            </p>
          </div>
          <button 
            type="button"
            onClick={onNavigateToSubscriptions}
            className="btn-primary bg-gradient-to-r from-amber-500 to-yellow-400 text-slate-950 hover:text-black font-extrabold px-6 py-3 cursor-pointer"
          >
            Become VIP Member (VIP प्लान एक्टिव करें)
          </button>
        </div>
      ) : (
